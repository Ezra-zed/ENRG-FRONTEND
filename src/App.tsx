...
function Signin() {
  ...
  return <div className="signin-modal-page">
    ...
    <section className="signin-modal" aria-label="Sign in to ENRG">
      ...
      <div className="signin-modal-brand"><span className="signin-modal-mark"><Sun size={27} /></span><div><p className="text-xl font-bold tracking-tight">Sign in to ENRG</p><p className="mt-1 text-sm text-muted-foreground">Your solar journey starts here.</p></div></div>
      {method === 'O-auth' && <><button type="button" data-testid="button-signin-method-O-auth" onClick={submit} disabled={mutation.isPending} className="signin-google-button"><span className="signin-google-icon">G</span>{mutation.isPending ? 'Signing you in...' : 'Continue with Google'}</button><button type="button" data-testid="button-signin-email" onClick={() => setMethod('JWT-auth')} className="signin-email-button"><span className="signin-mail-icon"><Send size={18} /></span>Sign in with Email</button><div className="signin-divider"><span>or</span></div><button type="button" data-testid="button-signin-method-no-password" onClick={() => setMethod('no-password')} className="signin-phone-link">Sign in with a one-time code</button></>}
