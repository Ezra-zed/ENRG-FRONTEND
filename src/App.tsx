...
import {
  ...
  ShoppingBag, Sparkles, Sun, Upload, UserRound, Users, X, Zap,
} from 'lucide-react';
import {
  ...
  const visibleNav = isCompany ? companyNav : user?.role === 'admin' ? adminNav : nav;
    ...
    <header className="sticky top-0 z-40 border-b border-border/70 bg-background/90 backdrop-blur-xl">
      <div className="mx-auto flex h-[72px] max-w-[1320px] items-center justify-between px-5 lg:px-8">
        <div className="flex items-center gap-9"><Logo /><nav className="hidden items-center gap-2 md:flex">{visibleNav.map(item => { const active = location === item.href || (item.href !== '/' && location.startsWith(`${item.href}/`)); return <Link key={item.href} href={item.href} data-testid={`link-nav-${item.label.toLowerCase().replaceAll(' ', '-')}`} className={`rounded-full px-3.5 py-2 text-sm font-semibold ${active ? 'bg-[#dfece0] text-accent shadow-sm' : 'text-muted-foreground hover:bg-secondary hover:text-foreground'}`}>{active && <span className="mr-2 inline-block size-1.5 rounded-full bg-primary align-middle" />}{item.label}</Link>; })}</nav></div>
      ...
      {open && <div className="border-t border-border bg-card p-4 md:hidden"><div className="grid gap-1">{visibleNav.map(item => { const active = location === item.href || (item.href !== '/' && location.startsWith(`${item.href}/`)); return <Link key={item.href} href={item.href} data-testid={`link-mobile-${item.label.toLowerCase().replaceAll(' ', '-')}`} onClick={() => setOpen(false)} className={`rounded-xl px-3 py-3 text-sm font-semibold ${active ? 'bg-[#dfece0] text-accent' : 'hover:bg-secondary'}`}>{item.label}</Link>; })}{user && <button type="button" data-testid="button-mobile-logout" onClick={logout} className="mt-2 rounded-xl px-3 py-3 text-left text-sm font-semibold text-muted-foreground hover:bg-secondary">Log out</button>}</div></div>}
    </header>
    <main>{children}</main>
...
function QuotePage() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (form.monthlyBill) formData.append('monthlyBill', String(form.monthlyBill));
...
function CompanyProfile() {
  ...
  if (!profile) {
    return <CompanyShell><PageHeader eyebrow="Company profile" title="Finish your company profile." description="Add the details homeowners need before choosing your business." /><Button onClick={() => setLocation('/company/profile/setup')}><Plus size={16} /> Complete profile</Button></CompanyShell>;
  }

  return <CompanyShell><PageHeader eyebrow="Company profile" title="Your company profile is ready." description="This is the information homeowners will use to understand your business." /><div className="max-w-3xl rounded-3xl border border-border bg-card p-6 shadow-[var(--shadow-card)] sm:p-8"><div className="grid gap-6 sm:grid-cols-2"><ProfileDetail label="Installation experience" value={`${profile.installExperienceYears || '0'} years`} /><ProfileDetail label="Service locations" value={profile.serviceLocations} /><ProfileDetail label="Products" value={profile.products} /><ProfileDetail label="Brands" value={profile.brands} /></div><div className="mt-6 border-t border-border pt-6"><p className="text-sm font-semibold">Pricing packages</p><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-muted-foreground">{profile.pricingPackages || 'Not provided'}</p></div></div></CompanyShell>;
}

function ProfileDetail({ label, value }: { label: string; value?: string }) {
  return <div><p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">{label}</p><p className="mt-2 text-base font-semibold">{value || 'Not provided'}</p></div>;
}
...
function CompanyAssistant() {
  ...
  return <div className="company-assistant"><div className="company-assistant-glow" /><div className="relative"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-[#b8f2d0]">ENRG assistant</p><h2 className="mt-2 font-display text-2xl font-bold text-white">A second mind for your next move.</h2></div><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-white/10 text-[#b8f2d0]"><Sparkles size={19} /></span></div><p className="mt-3 text-sm leading-6 text-white/65">{reply}</p><div className="mt-5 flex flex-wrap gap-2">{suggestions.map(prompt => <button key={prompt} type="button" onClick={() => respond(prompt)} className="rounded-full border border-white/15 px-3 py-2 text-left text-xs font-semibold text-white/80 hover:border-[#b8f2d0]/70 hover:text-[#b8f2d0]">{prompt}</button>)}</div><form className="mt-4 flex gap-2" onSubmit={e => { e.preventDefault(); if (message.trim()) respond(message.trim()); }}><input aria-label="Ask ENRG assistant" value={message} onChange={e => setMessage(e.target.value)} placeholder="Ask your assistant..." className="min-w-0 flex-1 rounded-xl border border-white/15 bg-white/10 px-3 py-2.5 text-sm text-white outline-none placeholder:text-white/45 focus:border-[#b8f2d0]" /><button type="submit" aria-label="Send message" className="grid size-11 shrink-0 place-items-center rounded-xl bg-[#b8f2d0] text-[#123b2b] hover:bg-white"><Send size={16} /></button></form></div></div>;
}

function CompanyDashboardLegacy() {
  const query = useGetCompanyMetrics({ query: { queryKey: getGetCompanyMetricsQueryKey() } });
  const m = query.data;
  const metrics = [{ label: 'Total leads', value: m?.totalLeads ?? 0, icon: Users }, { label: 'Ongoing projects', value: m?.activeLeads ?? 0, icon: ClipboardList }, { label: 'Quotes submitted', value: m?.quotesSubmitted ?? 0, icon: Send }, { label: 'Won projects', value: m?.wonProjects ?? 0, icon: Check }];
...
function CompanyDashboard() {
  ...
  const m = query.data;
  const metrics = [{ label: 'Total leads', value: m?.totalLeads ?? 0, icon: Users }, { label: 'Ongoing projects', value: m?.activeLeads ?? 0, icon: ClipboardList }, { label: 'Quotes submitted', value: m?.quotesSubmitted ?? 0, icon: Send }, { label: 'Won projects', value: m?.wonProjects ?? 0, icon: Check }];
  return <CompanyShell><PageHeader eyebrow="Company overview" title="A good day to grow." description="Your business, in the moments that matter." action={<Link href="/company/leads" data-testid="link-dashboard-leads" className="inline-flex items-center gap-2 rounded-full bg-accent px-4 py-2.5 text-sm font-bold text-accent-foreground">View leads <ArrowRight size={16} /></Link>} /><QueryState loading={query.isLoading} error={query.error} onRetry={() => query.refetch()}><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{metrics.map(metric => <div key={metric.label} className="rounded-2xl border border-border bg-card p-5 shadow-[var(--shadow-card)]"><div className="flex items-center justify-between"><span className="grid size-9 place-items-center rounded-xl bg-secondary text-accent"><metric.icon size={18} /></span><span className="text-xs font-bold text-accent">LIVE</span></div><p className="mt-6 font-display text-4xl font-bold">{metric.value}</p><p className="mt-1 text-sm text-muted-foreground">{metric.label}</p></div>)}</div><div className="mt-6 grid gap-6 lg:grid-cols-[1.2fr_.8fr]"><div className="rounded-3xl bg-accent p-7 text-accent-foreground"><p className="text-xs font-bold uppercase tracking-widest text-primary">Pipeline value</p><p className="mt-2 font-display text-5xl font-bold">{money(m?.pipelineValue)}</p><p className="mt-3 max-w-sm text-sm leading-6 text-accent-foreground/70">Every clear conversation is a project that can move forward.</p></div><CompanyAssistant /></div></QueryState></CompanyShell>;
}
