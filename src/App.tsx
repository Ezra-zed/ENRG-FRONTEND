...
function Signup() {
  ...
  const submit = (e: React.FormEvent) => { e.preventDefault(); mutation.mutate({ data: { ...form, role, ...(role === 'user' ? {} : { businessName: form.businessName, gstin: form.gstin, licenseNumber: form.licenseNumber }) } }, { onSuccess: session => { localStorage.setItem('enrg_user', JSON.stringify(session.user)); setLocation(role === 'user' ? '/customer/dashboard' : '/company/profile'); } }); };
...
function Signin() {
  ...
  const update = (key: keyof typeof form) => (e: React.ChangeEvent<HTMLInputElement>) => setForm({ ...form, [key]: e.target.value });
  const submit = (e: React.FormEvent) => { e.preventDefault(); const data = method === 'JWT-auth' ? { method, email: form.email, password: form.password } : method === 'no-password' ? { method, phone: form.phone, otp: form.otp } : { method, oauthProvider: 'google' as const, oauthToken: 'google-browser-session' }; mutation.mutate({ data }, { onSuccess: session => { localStorage.setItem('enrg_user', JSON.stringify(session.user)); setLocation(session.user.role === 'user' ? '/customer/dashboard' : session.user.role === 'admin' ? '/admin/dashboard' : '/company/dashboard'); } }); };
